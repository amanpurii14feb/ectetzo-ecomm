...
export function AddressManager({initial}:{initial:Address[]}){
 ...
 async function makeDefault(a:Address){const r=await fetch(`/api/addresses/${a.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({...a,isDefault:true})});if(r.ok)setRows(x=>x.map(i=>({...i,isDefault:i.id===a.id}))) }
 return <section className="address-manager">
  {rows.length?<div className="address-grid">{rows.map(a=><article className={`address-card ${a.isDefault?"is-default":""}`} key={a.id}>
   ...
   <footer><button onClick={()=>open(a)}><Pencil/> Edit</button>{!a.isDefault&&<button onClick={()=>makeDefault(a)}><Check/> Set default</button>}<button onClick={()=>remove(a.id)} className="danger"><Trash2/> Delete</button></footer>
  </article>)}<button onClick={()=>open()} className="address-add-card"><Plus/><b>Add new address</b><small>Save another delivery location</small></button></div>:<div className="address-empty"><span><MapPin/></span><h2>No saved addresses</h2><p>Add an address for a faster checkout experience.</p><button onClick={()=>open()} className="btn btn-yellow"><Plus/> Add new address</button></div>}
  <section className="delivery-preferences card"><div><MapPin/><span><b>Delivery preferences</b><small>Default address: {rows.find(x=>x.isDefault)?.label??"Not selected"}</small></span></div><p>Preferred instruction: Call before delivery</p></section>
  {editing&&<div className="address-modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setEditing(null)}><form onSubmit={save} className="address-modal" role="dialog" aria-modal="true" aria-labelledby="address-dialog-title">
   ...
   <div className="address-form">{error&&<p className="address-error">{error}</p>}
    <label><span className="label">Address label</span><input required minLength={2} maxLength={20} className="field" placeholder="Home, Office…" value={form.label} onChange={e=>update("label",e.target.value)}/></label>
    <label><span className="label">Full name</span><input required minLength={2} maxLength={80} autoFocus className="field" autoComplete="name" value={form.name} onChange={e=>update("name",e.target.value)}/></label>
    <label><span className="label">Mobile number</span><div className="phone-field"><span>+91</span><input required minLength={10} maxLength={10} pattern="[6-9][0-9]{9}" className="field" inputMode="numeric" autoComplete="tel" placeholder="10-digit number" value={form.phone} onChange={e=>update("phone",e.target.value.replace(/\D/g,"").slice(0,10))}/></div></label>
    <label className="wide"><span className="label">Street address</span><textarea required minLength={8} maxLength={200} className="field" autoComplete="street-address" rows={3} value={form.line1} onChange={e=>update("line1",e.target.value)}/></label>
    ...
    <label><span className="label">PIN code</span><input required minLength={6} maxLength={6} pattern="[1-9][0-9]{5}" className="field" inputMode="numeric" autoComplete="postal-code" value={form.pin} onChange={e=>update("pin",e.target.value.replace(/\D/g,"").slice(0,6))}/></label>
